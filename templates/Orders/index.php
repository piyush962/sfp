<div class="right-content-wrapper order-right-cnt">
                ...
                <div class="col-lg-6">
                    <div class="info_wrapper customer_info">
                        <h3>Customers Info</h3>
                            <input type="hidden" id="customerhiddenid" value="<?= $customerDetails['id']?>">
                        <ul>
                            <li>
                                <h6>Company Name</h6>
 ...
 ?>
            ...
            <div class="add-new-customer">
                <div class="filterField">
                    <select id="filterStatus" class="form-select" onchange="filterstatus(this)"> 
                                    <option value='' <?php echo $optionval == '' ? 'selected' : ''?>>Order Status</option>                                   
    ...
    ?>
                 <div class="filterField">
                   <input type="search" name="filterCustomer" id="filterCustomer" class="form-control"  onkeyup="searchcustomer(this.value)" placeholder="Search customer...">
                </div>
                <?php } ?>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody class="bodyCustomerListing">
                    <?php $count = $this->Paginator->counter('{{start}}'); 
                //    pr($this->Paginator->current());die;
...
function filterstatus(e) {
    var option = $(e).val();
var customerhiddenid = $('#customerhiddenid').val();
if(customerhiddenid > 0){
	if(option != ''){
            window.location.href = "/orders/index/"+customerhiddenid+"?optionval="+option;
        }else{
            window.location.href = "/orders/index/"+customerhiddenid;
        }
}else{
	if(option != ''){
    ...
    function searchcustomer(key){
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                console.log(res)
                $('.bodyCustomerListing').html(result[0]);
                $('.paginator-ajax').html(result[1])
            },
            error:function(err){
                alert(err);
            }
