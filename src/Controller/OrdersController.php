...
class OrdersController extends AppController
{
    ...
    {        
        ...
        if($customerid > 0){
            ...
            elseif(isset($_GET['optionval']) && $_GET['optionval'] >= 0){
                $ConditionArray['Orders.status'] = $_GET['optionval'];
        ...
        if($customerid > 0){
            ...
            $productCategoryData = $query->toArray();
        }else{
            $productCategoryData='';
        }
     if(isset($_GET['optionval']) && $_GET['optionval'] >= 0){
        ...
        $pageTitle='Orders'; 
        $this->set(compact('pageTitle', 'orderData', 'customerDetails','optionval','productCategoryData'));
    ...
    { 
        ...
        if($this->request->is(['post','put'])){
            ...
            $postData['unit_price']=$postData['unit_price'];
            $postData['total_price']=$postData['unit_price']*$postData['quantity']; 
    ...
    if($newcategoryentity->status==0 ){
        ...
        $categoryTable->saveMany([$newcategoryentity]);
    ...
    public function changeorderstatus($orderId=null,$optionval=null){
        ...
        if($orderSaved){
            ...
            if($orderSaved['status'] == 1){
                $notifyData['client_message']='Your reorder request with '.$orderSaved['packaging_name'].' has been approved with #'.$orderSaved['id'];
            }elseif($orderSaved['status'] == 3){
                $notifyData['client_message']= 'Your order '.$orderSaved['packaging_name'].' is delivered with #'.$orderSaved['id'];
            }elseif($orderSaved['status'] == 4){
                $notifyData['client_message']= 'Your order '.$orderSaved['packaging_name'].' is canceled with #'.$orderSaved['id'];
            }
    ...
    public function search(){
        $this->viewBuilder()->setLayout('ajax');
         ...
         if($this->request->is('post')){
             ...
             $whereArray = [
                'Users.name LIKE' => '%' . $Data['key'] . '%'
            ];
            
            if ($Data['customerid'] > 0) {
                $whereArray[' Orders.client_id'] = $Data['customerid'];
            }
            elseif ($Data['selectedstatus'] >= 0) {
                $whereArray[' Orders.status'] = $Data['selectedstatus'];
            }           
            // pr($whereArray);die;
